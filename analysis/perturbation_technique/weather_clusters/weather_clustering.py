...
N_CLUSTERS = [4]         # Number of clusters for individual variables (ppt, tmin, tmax)
...
def fuzzy_c_means(X, n_clusters, m=1.2, max_iter=100, tol=1e-5, 
    ...
    Parameters:
      ...
      device      : GPU device to use (e.g., device).
      seed        : Random seed.
...
def main():
    ...
    # 7. Perform k-means clustering on each variable using device
    ppt_torch  = torch.from_numpy(ppt_flat_scaled).float().to(device)
    tmin_torch = torch.from_numpy(tmin_flat_scaled).float().to(device)
    tmax_torch = torch.from_numpy(tmax_flat_scaled).float().to(device)
    combined_data_torch = torch.from_numpy(combined_data_flat_scaled).float().to(device)
    ...
    for technique in technique_region:
        ...
        for n_clusters in N_CLUSTERS:
            if technique == "kmeans":
                ppt_cluster_ids, _ = run_kmeans(ppt_torch, n_clusters + 1, device, 42)
                tmin_cluster_ids, _ = run_kmeans(tmin_torch, n_clusters + 1, device, 42)
                tmax_cluster_ids, _ = run_kmeans(tmax_torch, n_clusters + 1, device, 42)
                combined_cluster_ids, _ = run_kmeans(combined_data_torch, n_clusters + 1, device, 42)
            elif technique == "fcm":
                ppt_cluster_ids, _ = fuzzy_c_means(ppt_torch, n_clusters=n_clusters+1, device=device, seed=42)
                tmin_cluster_ids, _ = fuzzy_c_means(tmin_torch, n_clusters=n_clusters+1, device=device, seed=42)
                tmax_cluster_ids, _ = fuzzy_c_means(tmax_torch, n_clusters=n_clusters+1, device=device, seed=42)
                combined_cluster_ids, _ = fuzzy_c_means(combined_data_torch, n_clusters=n_clusters+1, device=device, seed=42)
            elif technique == "gmm":
                ppt_cluster_ids, _ = run_gmm_torch(ppt_torch, n_clusters=n_clusters+1, device=device, seed=42)
                tmin_cluster_ids, _ = run_gmm_torch(tmin_torch, n_clusters=n_clusters+1, device=device, seed=42)
                tmax_cluster_ids, _ = run_gmm_torch(tmax_torch, n_clusters=n_clusters+1, device=device, seed=42)
                combined_cluster_ids, _ = run_gmm_torch(combined_data_torch, n_clusters=n_clusters+1, device=device, seed=42)
    ...
    for technique, technique_clusters in precomputed_clusters.items():
        ...
        for n_clusters in N_CLUSTERS:
            ...
            for key, cluster_np in clusters_dict.items():
                ...
                # 1) Make an output directory per technique/variable/cluster-count
                out_dir = os.path.join("outputs", technique, f"{key}_{n_clusters}")
                os.makedirs(out_dir, exist_ok=True)

                # 2) Save the vector file (shapefile)
                shp_path = os.path.join(out_dir, f"{key}_{n_clusters}.shp")
                gdf.to_file(shp_path)
                print(f"Saved shapefile: {shp_path}")

                # 3) Save a PNG of the raw cluster raster
                png_path = os.path.join(out_dir, f"{key}_{n_clusters}.png")
                fig, ax = plt.subplots(1, 1, figsize=(8, 6))
                ax.imshow(cluster_np, cmap="tab20", interpolation="none")
                ax.set_title(f"{technique.upper()} – {key} ({n_clusters} clusters)")
                ax.axis("off")
                plt.tight_layout()
